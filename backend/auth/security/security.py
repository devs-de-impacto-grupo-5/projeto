    ...
    def role_checker(current_user: User = Depends(get_current_user)):
        if current_user.role != required_role:
            raise HTTPException(
                ...
                detail=f"Acesso restrito a usuários com role '{required_role}'"
            )
        return current_user
    return role_checker
...
def require_any_role(required_roles: List[str]):
    ...
    def role_checker(current_user: User = Depends(get_current_user)):
        if current_user.role not in required_roles:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Acesso requerido para os roles: {', '.join(required_roles)}"
            )
        return current_user
    return role_checker
...
# Você pode criar verificadores de role específicos quando necessário
# Exemplo: get_current_admin = require_role("admin")